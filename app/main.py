...
prediction_latency = Histogram(
    ...
    'Latency of churn predictions'
)

high_risk_gauge = Gauge(
    'churn_high_risk_customers',
    'Number of high risk customers in current batch'
)
...
async def predict(customer: CustomerData):
    ...
    try:
        ...
        # Make prediction
        probability, prediction, confidence, risk_level = predictor.predict(customer.dict())
        ...
        logger.info(f"Prediction made for {customer_id}: {prediction} (risk: {risk_level})")
...
async def batch_predict(request: BatchPredictionRequest):
    ...
    try:
        ...
        logger.info(f"Batch prediction completed: {len(predictions)} customers, {high_risk_count} high risk")
...
async def model_info():
    ...
    return {
        ...
        "features": predictor.feature_names[:10] if predictor.feature_names else []  # Show first 10 features
    }
